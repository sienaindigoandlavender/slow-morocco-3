  ...
  return (
    <div>
      {content.split(/\n\n+/).filter(Boolean).map((p, i) => {
        ...
        return <p key={i} className="mb-6 leading-[1.85]" style={{ fontSize: 15, color: "#262626" }}>{p}</p>;
      })}
    </div>
  );
}
...
// ─────────────────────────────────────────────
// CHART 1 — Marriage vs Divorce trend
// ─────────────────────────────────────────────
...
function MarriageDivorceTrend() {
  ...
  useEffect(() => {
    ...
    const pathM = g.append("path").datum(MARRIAGE_DIVORCE)
      .attr("fill", "none").attr("stroke", "#c9a96e").attr("stroke-width", 2)
      .attr("d", lineM);
    ...
    const pathD = g.append("path").datum(MARRIAGE_DIVORCE)
      .attr("fill", "none").attr("stroke", "#c04040").attr("stroke-width", 2)
      .attr("d", lineD);
    ...
    MARRIAGE_DIVORCE.forEach(d => {
      g.append("circle").attr("cx", x(d.year)).attr("cy", y(d.marriages))
        .attr("r", 4).attr("fill", "#c9a96e").attr("cursor", "pointer")
        .on("mouseenter", () => setHov(d)).on("mouseleave", () => setHov(null));
      g.append("circle").attr("cx", x(d.year)).attr("cy", y(d.divorces))
        .attr("r", 4).attr("fill", "#c04040").attr("cursor", "pointer")
        .on("mouseenter", () => setHov(d)).on("mouseleave", () => setHov(null));
    });
  ...
  return (
    <div ref={ref}>
      <div className="flex items-center gap-6 mb-4">
        ...
        {hov && (
          <span className="font-mono text-[11px] text-neutral-500 ml-auto">
            {hov.year} · <span className="text-neutral-800">{hov.marriages.toLocaleString()} marriages</span>
      ...
      <p className="font-mono text-[10px] text-neutral-300 mt-3">
        Source: HCP Morocco · Haut-Commissariat au Plan · 2024
      </p>
    </div>
  );
}
...
// ─────────────────────────────────────────────
// CHART 2 — Wedding cost breakdown (horizontal bars)
// ─────────────────────────────────────────────
...
function WeddingCostChart() {
  ...
  useEffect(() => {
    ...
    g.selectAll(".bar").data(sorted).enter()
      ...
      .attr("x", 0).attr("rx", 2)
      .attr("fill", tier === "low" ? "#4a9aba" : tier === "mid" ? "#c9a96e" : "#2d7a5a")
      .attr("width", 0)
    ...
    g.selectAll(".val").data(sorted).enter()
      ...
      .attr("opacity", 0)
      .text(d => `${(d[tier] / 1000).toFixed(0)}k MAD`)
      .transition().duration(400).delay((_, i) => i * 60 + 400)
      .attr("opacity", 1);
...
// ─────────────────────────────────────────────
// CHART 3 — Divorce by consent over time
// ─────────────────────────────────────────────
...
function DivorceConsentChart() {
  ...
  useEffect(() => {
    ...
    const W = 500, H = 200;
    const m = { top: 10, right: 20, bottom: 32, left: 44 };
    const w = W - m.left - m.right;
    const h = H - m.top - m.bottom;
    ...
    g.append("path").datum(DIVORCE_TYPE)
      .attr("fill", "#2d7a5a").attr("fill-opacity", 0.12).attr("d", area);
    ...
    // End label
    g.append("text")
      .attr("x", x(2024) + 4).attr("y", y(89.3) + 4)
      .attr("font-family", "monospace").attr("font-size", 11).attr("fill", "#2d7a5a")
      .text("89.3%");
  ...
  return (
    <div ref={ref}>
      <p className="font-mono text-[10px] uppercase tracking-wider text-neutral-400 mb-3">
        Consensual divorces as % of all divorces
      </p>
      <svg ref={svgRef} className="w-full" />
...
// ─────────────────────────────────────────────
// CHART 4 — Singlehood rate & marriage age
// ─────────────────────────────────────────────
...
function SinglehoodChart() {
  ...
  useEffect(() => {
    ...
    const W = 500, H = 200;
    const m = { top: 10, right: 20, bottom: 32, left: 40 };
    const w = W - m.left - m.right;
    const h = H - m.top - m.bottom;
    ...
    g.append("path").datum(SINGLEHOOD_WOMEN)
      .attr("fill", "#9b8db0").attr("fill-opacity", 0.12).attr("d", area);
    ...
    g.append("text").attr("x", x(2024) - 60).attr("y", y(11.1) - 5)
    ...
    <div className="min-h-screen bg-white text-neutral-900">
      ...
      <section>
        {story.hero_image ? (
          <div className="relative w-full" style={{ height: "70vh", minHeight: 480 }}>
            <Image src={story.hero_image} alt={story.title} fill className="object-cover" priority />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {images.map((img, i) => (
              <Reveal key={i} delay={i * 0.08}>
                <figure>
                  <div className="relative w-full overflow-hidden" style={{ aspectRatio: "16/9", background: "#f0f0f0" }}>
        ...
        <Reveal>
          <div className="flex items-center justify-between gap-8 flex-wrap">
            <p className="font-serif italic text-xl text-neutral-600 max-w-lg">
              The Morocco that matters isn't on the wedding circuit. We'll take you to the rest of it.
            </p>
      ...
      <section className="px-8 md:px-[8%] lg:px-[12%] py-16" style={{ background: "#141414" }}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-4xl">
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.2em] mb-4" style={{ color: "rgba(255,255,255,0.25)" }}>Data sources</p>
            <div className="text-[11px] leading-[1.8]" style={{ color: "rgba(255,255,255,0.4)" }}>
              <p>HCP — Haut-Commissariat au Plan du Maroc. <em>Femmes Marocaines en Chiffres 2024</em>.</p>
              <p>Conseil Supérieur du Pouvoir Judiciaire (CSPJ). Marriage and divorce statistics 2017–2024.</p>
              <p>Ministère de la Justice. Family Code reform proposals, December 2024.</p>
            </div>
          </div>
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.2em] mb-4" style={{ color: "rgba(255,255,255,0.25)" }}>Story sources</p>
            <div className="text-[11px] leading-[1.8]" style={{ color: "rgba(255,255,255,0.4)" }}>
              {sources.length > 0
                ? sources.map((s, i) => <p key={i}>{s}</p>)
                : <p>Field interviews, Marrakech 2023–2024. Bewildered in Morocco. Morocco World News.</p>
              }
            </div>
          </div>
        </div>
