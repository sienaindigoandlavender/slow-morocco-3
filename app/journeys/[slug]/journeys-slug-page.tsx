...
async function getJourneyData(slug: string) {
  ...
  const journey: Journey = {
    ...
    description: (journeyData.short_description || "").replace(/<br\s*\/?>/gi, '\n'),
    arcDescription: (journeyData.arc_description || "").replace(/<br\s*\/?>/gi, '\n'),
  ...
  const itinerary: ItineraryDay[] = routeIds.map(
    (routeId: string, index: number) => {
      ...
      return {
        ...
        description: (route.route_narrative || "").replace(/<br\s*\/?>/gi, '\n'),
        imageUrl,
